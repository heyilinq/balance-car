/**
 * ============================================================
 * B570 平衡小车 - 软件I2C驱动（GPIO模拟I2C时序）
 * ============================================================
 * 引脚: PB8=SCL(时钟), PB9=SDA(数据)
 * 不使用STM32硬件I2C（F103硬件I2C有已知Bug，容易卡死）
 * 位带操作: PBout/PBin 直接操控寄存器，速度远快于HAL函数
 *
 * 核心函数:
 *   i2cWrite(addr, reg, len, data) - 写设备寄存器
 *   i2cRead(addr, reg, len, buf)   - 读设备寄存器
 * 上层调用: MPU6050驱动通过这两个函数与传感器通信
 * ============================================================
 */
